...
## copilot-config Repo Workflow

> These rules apply only when working in the `copilot-config` repo itself.

- **Push to `master` directly** — no feature branches or PRs. This is a personal config repo.
- **No tests or CI** — just commit and push.
- **File layout:**
  - `.copilot/copilot-instructions.md` — global instructions (symlinked to `~/.copilot/`)
  - `.copilot/mcp-config.json` — MCP servers (symlinked; `mcp.json` is an alias)
  - `.copilot/lsp-config.json` — LSP servers (symlinked)
  - `.copilot/config.portable.json` — portable settings (patched into `config.json`, NOT symlinked)
...
- **Adding a new tracked config file** requires updating **4 places:**
  1. `$configFileLinks` array in `setup.ps1`
  2. `CONFIG_FILE_LINKS` array in `setup.sh`
  3. Backup file list in `setup.ps1` (~line 292)
  4. Backup file list in `setup.sh` (~line 442)
- **Checking for uncommitted work:** don't just run `git status` — also compare `~/.copilot/` against the repo for new untracked files that should be adopted.
- **Secrets:** MCP config uses `${VAR_NAME}` env var syntax. Never hardcode API keys.
